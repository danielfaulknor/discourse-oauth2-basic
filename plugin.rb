...
class OAuth2BasicAuthenticator < ::Auth::OAuth2Authenticator
  ...
  def fetch_user_details(token, id)
    ...
    user_url = "#{user_json_url}?access_token=#{token}"

    log("user_json_url: #{user_json_method} #{user_url}")

    bearer_token = "Bearer #{token}"
    user_json_response =
      if user_json_method.downcase.to_sym == :post
        Net::HTTP
          .post_form(URI(user_url), { 'Authorization' => bearer_token })
          .body
      else
        open(user_url, 'Authorization' => bearer_token).read
      end
